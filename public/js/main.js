...
fetch('/api/views')
  .then(res => res.json())
  .then(data => {
    const uVis = document.getElementById('stat-unique-visitors');
    const tVis = document.getElementById('stat-total-views');
    if (uVis && data.uniqueVisitors !== undefined) {
      uVis.dataset.target = data.uniqueVisitors;
      if (uVis.textContent === '0' || uVis.textContent === '') {
        animCounter(uVis, data.uniqueVisitors, '', 0);
      }
    }
    if (tVis && data.totalViews !== undefined) {
      tVis.dataset.target = data.totalViews;
      if (tVis.textContent === '0' || tVis.textContent === '') {
        animCounter(tVis, data.totalViews, '', 0);
      }
    }
  })
  .catch(err => console.error('Failed to fetch view metrics:', err));
